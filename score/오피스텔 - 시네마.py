...
# 아파트 쿼리 실행
apt_query = "SELECT id, sigungu, lat, lng FROM apt"
cursor.execute(apt_query)
apt = cursor.fetchall()

# 영화관 쿼리 실행
cinema_query = "SELECT id, name, lat, lng FROM cinema"
...
for a in apt:
    ...
    score_query = f"UPDATE score SET cinema = %s WHERE id = {home_id} AND type='APT'"
