...
# 빌라 쿼리 실행
villa_query = "SELECT id, sigungu, lat, lng FROM officetel"
cursor.execute(villa_query)
villa = cursor.fetchall()

# 도서관 쿼리 실행
library_query = "SELECT id, name, lat, lng FROM park"
...
for a in villa:
    ...
    score_query = f"UPDATE score SET park = %s WHERE id = {home_id} AND type='OFFICETEL'"
